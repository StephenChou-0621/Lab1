...
const osSemaphoreAttr_t timerSem_attributes = {
  .name = "timerSem"
};
/* USER CODE BEGIN PV */
...
int main(void)
{
  ...
  myTask02Handle = osThreadNew(StartTask2, NULL, &myTask02_attributes);
...
void StartTask2(void *argument)
{
  ...
  /* Infinite loop */
  osSemaphoreAcquire(timerSemHandle, 0);                // 清掉初始值
  __HAL_TIM_CLEAR_FLAG(&htim7, TIM_FLAG_UPDATE);        // 清掉殘留旗標
  HAL_TIM_Base_Start_IT(&htim7);                        // 啟動 10 秒計時
  for(;;)
  {
    osSemaphoreAcquire(timerSemHandle, osWaitForever);  // 等 10 秒到
    osMutexAcquire(ledMutexHandle, osWaitForever);      // 拿鑰匙
    for (int i = 0; i < 20; i++)                        // 10 Hz x 2 秒
    {
      HAL_GPIO_WritePin(GPIOB, LED2_Pin, GPIO_PIN_SET);
      osDelay(50);
      HAL_GPIO_WritePin(GPIOB, LED2_Pin, GPIO_PIN_RESET);
    ...
    osMutexRelease(ledMutexHandle);                     // 還鑰匙
  }
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  {
    osSemaphoreRelease(timerSemHandle); // release sem. with id timerSemHandle, paired with the task acquiring that sem. id.
  }
